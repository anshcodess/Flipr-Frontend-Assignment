...
const displayWinners = async () => {
  ...
  let dataDisplay = dataDisplayObj[0].laureates
    .map((object) => {
      return `
...
const allLaureates = async () => {
  ...
  for (var i = 0; i < payload.prizes.length; i++) {
    let dataDisplay = payload.prizes[i].laureates
      .map((object) => {
        return `
            <div class="winnerDetails-container">
                <p>Category: ${payload.prizes[i].category}</p>
                <p>Year: ${payload.prizes[i].year}</p>
                <p>Name: ${object.firstname} ${object.surname}</p>
                <p>Motivation: ${object.motivation}</p>
            </div>
            `;
